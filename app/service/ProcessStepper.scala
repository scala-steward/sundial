...
class ProcessStepper @Inject() (
  ...
  def step(process: Process, dao: SundialDao, metrics: SundialMetrics): Process = {
    Logger.info(s"Stepping ${process}")
    metrics.steps.incrementAndGet()
    val taskDefinitions = dao.processDefinitionDao.loadTaskDefinitions(process.id).filter { taskDefinition =>
    ...
    val baseTasks = dao.processDao.loadTasksForProcess(process.id).filter { task =>
      process.taskFilter match {
    ...
    val hasKillRequest = !dao.triggerDao.loadKillProcessRequests(process.id).isEmpty
    val currentlyRunning = tasks.filterNot(_.status.isComplete)
    val taskContext = new TaskContext(tasks, taskDefinitions, process, hasKillRequest)
    val byStatus = taskDefinitions.groupBy { taskDefinition =>
      taskContext.taskExecutionCondition(taskDefinition)
    }
    val allowedToRun = byStatus.getOrElse(TaskExecutionCondition.Allowed, Seq.empty)
    ...
    allowedToRun.foreach { taskDefinition =>
      ...
      val previousAttempts = taskContext.tasksForTaskDefinition(taskDefinition).size
      val task = dao.processDao.saveTask(Task(id = UUID.randomUUID(),
  ...
  private def killIfNecessary(task: Task, taskDefinition: TaskDefinition, dao: SundialDao): Task = {
    ...
    task.status match {
      case TaskStatus.Running() =>
        ...
        if(isOverTime || hasKillRequest) {
          ...
          val event = TaskEventLog(UUID.randomUUID(), task.id, new Date(), EVENT_SOURCE_KEY, reason)
            ...
            if (taskDefinition.requireExplicitSuccess) {
              ...
              dao.processDao.findReportedTaskStatus(task.id) match {
                case Some(ReportedTaskStatus(_, status)) =>
                  val message = s"The underlying executable has finished, and a status of $status was found; updating task."
                  ...
                  status
                case _ =>
                  val message = s"The underlying executable has finished, but no reported status was found; updating task as failed."
                  dao.taskLogsDao.saveEvent(
                    TaskEventLog(UUID.randomUUID(), task.id, new Date(), EVENT_SOURCE_KEY, message)
                  )
                  dao.ensureCommitted()
  ...
  class TaskContext(val allTasks: Seq[Task], val allTaskDefinitions: Seq[TaskDefinition], val process: Process, val killed: Boolean) extends service.TaskContext {
    ...
    override def allConditions(taskDefinition: TaskDefinition): Map[String, TaskExecutionCondition] = {
      // for debugging purposes only
      SortedMap(
        "taskExecutionCondition" -> taskExecutionCondition(taskDefinition),
        "taskNotCurrentlyRunningExecutionCondition" -> taskNotCurrentlyRunningExecutionCondition(taskDefinition),
        "taskNotAlreadySuccessfulCondition" -> taskNotAlreadySuccessfulCondition(taskDefinition),
        "taskMaxAttemptsExecutionCondition" -> taskMaxAttemptsExecutionCondition(taskDefinition),
        "taskDependenciesExecutionCondition" -> taskDependenciesExecutionCondition(taskDefinition),
        "taskBackoffExecutionCondition" -> taskBackoffExecutionCondition(taskDefinition),
        "processNotKilledExecutionCondition" -> processNotKilledExecutionCondition()
      )
    }

    override def mostRecentStatusForTaskDefinition(taskDefinition: TaskDefinition): Option[TaskStatus] = {
      tasksForTaskDefinition(taskDefinition)
        .headOption
        .map(_.status)
    }

    override def tasksForTaskDefinition(taskDefinition: TaskDefinition): Seq[Task] = {
      allTasks.filter(_.taskDefinitionName == taskDefinition.name)
        .sortBy(_.startedAt)
        .reverse
    }
    ...
    override def taskExecutionCondition(taskDefinition: TaskDefinition): TaskExecutionCondition = {
      val conditions = Seq(
    ...
    override def taskNotCurrentlyRunningExecutionCondition(taskDefinition: TaskDefinition): TaskExecutionCondition = {
    ...
    override def taskNotAlreadySuccessfulCondition(taskDefinition: TaskDefinition): TaskExecutionCondition = {
      mostRecentStatusForTaskDefinition(taskDefinition) match {
        case Some(TaskStatus.Success(_)) => TaskExecutionCondition.CompletedOrProhibited
        case _ => TaskExecutionCondition.Allowed
      }
    }
        ...
        mostRecentStatusForTaskDefinition(requiredDefinition) match {
          ...
          case _ =>
            ...
            taskExecutionCondition(requiredDefinition) match {
              case TaskExecutionCondition.Allowed => TaskExecutionCondition.Pending
              case TaskExecutionCondition.Pending => TaskExecutionCondition.Pending
              case TaskExecutionCondition.CompletedOrProhibited => TaskExecutionCondition.CompletedOrProhibited
            }
        }
      }
      val optionalConditions = optional.flatMap(taskDefinitionForName).map { optionalDefinition =>
        ...
        mostRecentStatusForTaskDefinition(optionalDefinition) match {
          ...
          case Some(_) => TaskExecutionCondition.Pending // dependency is running
          case None => TaskExecutionCondition.Pending // dependency hasn't tried to run yet
        }
      }
      (requiredConditions ++ optionalConditions).reduceOption(TaskExecutionCondition.reduce).getOrElse(TaskExecutionCondition.Allowed)
    }

    // If the task is awaiting retry, has enough time elapsed since its last failure?
    override def taskBackoffExecutionCondition(taskDefinition: TaskDefinition): TaskExecutionCondition = {
        ...
        case Some(task) =>
          task.status match {
            case status: CompletedTaskStatus =>
              ...
              val requiredMillis = TimeUnit.SECONDS.toMillis(taskDefinition.backoff.backoffTimeInSeconds(numberOfAttempts))
