...
class InMemoryTaskMetadataDao extends TaskMetadataDao {
  ...
  override def loadMetadataForTask(taskId: UUID): Seq[TaskMetadataEntry] = lock.synchronized {
    entries.filter(_.taskId == taskId).toList
  }

  override def saveMetadataEntries(entries: Seq[TaskMetadataEntry]): Unit = lock.synchronized {
