...
class DisplayModels @Inject() (graphify: Graphify, s3Client: AmazonS3, @Named("s3Bucket") s3Bucket: String) {
  ...
  def fetchProcessDto(processId: UUID, generateGraph: Boolean)(implicit dao: SundialDao): Option[ProcessDTO] = {
    for {
      ...
      taskDefs = dao.processDefinitionDao.loadTaskDefinitions(process.id).sortBy(_.name)
    } yield {
      ...
      ProcessDTO(
        ...
        endedAt = endedAt,
        durationStr = DateUtils.prettyDuration(process.startedAt, process.endedAt.getOrElse(new Date())),
        imageUrlOpt
      )
    }
  }

  def toTaskDto(taskDef: TaskDefinition, allTasks: Seq[Task])(implicit dao: SundialDao): TaskDTO = {
    val tasks = allTasks.filter(_.taskDefinitionName == taskDef.name).sortBy(_.startedAt)
    ...
    val logs = finalIdOpt.map(dao.taskLogsDao.loadEventsForTask).getOrElse(Seq.empty)
    val reason = finalOpt.flatMap {
      task => task.status match {
  ...
  def toProcessDefinitionDTO(processDefinition: ProcessDefinition)(implicit dao: SundialDao): ProcessDefinitionDTO = {
    val lastProcess = dao.processDao.findProcesses(processDefinitionName = Some(processDefinition.name),
                                                   limit = Some(1)).headOption
    ...
    val lastCompletedProcess = dao.processDao.findProcesses(processDefinitionName = Some(processDefinition.name),
                                                            statuses = Some(Seq(ProcessStatusType.Succeeded, ProcessStatusType.Failed)),
                                                            limit = Some(1)).headOption
    val processStartFormat = new SimpleDateFormat("M/dd H:mm z")
    val processHistory = dao.processDao.findProcesses(processDefinitionName = Some(processDefinition.name),
                                                      limit = Some(10)).map { process =>
      ProcessSummaryDTO(process.id,
                        process.status.statusType,
                        processStartFormat.format(process.startedAt),
                        process.endedAt.getOrElse(new Date).getTime - process.startedAt.getTime)
    }
    ...
    ProcessDefinitionDTO(processDefinition, nextRun, lastProcess, lastDuration, processHistory)
  }
