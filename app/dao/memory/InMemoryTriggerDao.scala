...
class InMemoryTriggerDao extends TriggerDao {
  ...
  private val processTriggerRequests =
    collection.mutable.HashMap[UUID, ProcessTriggerRequest]()
  private val taskTriggerRequests =
    collection.mutable.HashMap[UUID, TaskTriggerRequest]()
  private val processKillRequests =
    collection.mutable.HashMap[UUID, KillProcessRequest]()

  override def loadOpenProcessTriggerRequests(
      processDefinitionNameOpt: Option[String] = None) = lock.synchronized {
    val all =
      processTriggerRequests.values.filter(_.startedProcessId.isEmpty).toSeq
    processDefinitionNameOpt match {
      case Some(processDefinitionName) =>
        all.filter(_.processDefinitionName == processDefinitionName)
      case _ => all
    }
  }
  ...
  override def loadOpenTaskTriggerRequests(
      processDefinitionNameOpt: Option[String] = None) = lock.synchronized {
    val all =
      taskTriggerRequests.values.filter(_.startedProcessId.isEmpty).toSeq
    processDefinitionNameOpt match {
      case Some(processDefinitionName) =>
        all.filter(_.processDefinitionName == processDefinitionName)
      case _ => all
  ...
  override def saveKillProcessRequest(request: KillProcessRequest) =
    lock.synchronized {
