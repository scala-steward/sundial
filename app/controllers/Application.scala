...
class Application @Inject()(config: Configuration,
  ...
  def index = Action { implicit request =>
    daoFactory.withSundialDao { implicit dao =>
      ...
      val processDtos =
        processes.flatMap(displayModels.fetchProcessDto(_, false))
  ...
  def processDefinitions = Action { implicit request =>
    daoFactory.withSundialDao { implicit dao =>
      val processDefinitions =
        dao.processDefinitionDao.loadProcessDefinitions().sortBy(_.name)
    ...
    daoFactory.withSundialDao { implicit dao =>
      displayModels.fetchProcessDto(UUID.fromString(processId), false) match {
        case Some(processDto) =>
          val taskMetadata = processDto.tasks
            .flatMap(_.tasks)
            .map { task =>
              ...
              task.id -> metadata
                .groupBy(_.key)
                .mapValues(_.sortBy(_.when.getTime).last.value)
            }
            .toMap
  ...
  def processDefinition(processDefinitionName: String) = Action {
    implicit request =>
      daoFactory.withSundialDao { implicit dao =>
        dao.processDefinitionDao
          .loadProcessDefinition(processDefinitionName) match {
          case Some(processDefinition) =>
            // TODO Allow count to be passed in as a parameter
            val processes =
              dao.processDao
                .findProcesses(processDefinitionName =
                                 Some(processDefinitionName),
                               limit = Some(100))
                .flatMap(proc => displayModels.fetchProcessDto(proc.id, false))
            val taskDefs = dao.processDefinitionDao.loadTaskDefinitionTemplates(
              processDefinitionName)
            val triggers = dao.triggerDao
              .loadOpenProcessTriggerRequests()
              .filter(_.processDefinitionName == processDefinitionName)
            Ok(
              views.html.processDefinition(processDefinition,
                                           taskDefs,
                                           processes,
                                           triggers))
          case _ =>
  ...
  private def readEntireEntry(
      tarArchiveInputStream: TarArchiveInputStream): String = {
    val bytes = IOUtils.toByteArray(tarArchiveInputStream)
    new String(bytes)
  }

  private def readTruncatedEntry(tarArchiveInputStream: TarArchiveInputStream,
                                 totalSize: Int,
                                 maxLogSizeBytes: Int): String = {
