...
trait TaskContext {
  ...
  def mostRecentStatusForTaskDefinition(
  ...
  def taskExecutionCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition

  def taskNotCurrentlyRunningExecutionCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition

  def taskNotAlreadySuccessfulCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition

  def taskMaxAttemptsExecutionCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition

  def taskDependenciesExecutionCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition

  def taskBackoffExecutionCondition(
      taskDefinition: TaskDefinition): TaskExecutionCondition
