...
@deprecated(
  message =
    "ECS has been deprecated in favour of AWS Batch that allows easier way of scaling and cheaper execution thanks to spot instances.",
  since = "2.0.0"
)
class ECSServiceExecutor @Inject()(config: Configuration,
                                   ...
                                   @Named("sdbDomain") sdbDomain: String)
    extends SpecificTaskExecutor[ECSExecutable, ECSContainerState] {
  ...
  private def buildLogPaths(executable: ECSExecutable) = {
    val logDirectories = executable.logPaths
      .map(FilenameUtils.getFullPathNoEndSeparator)
      .distinct
      .sorted
    logDirectories.zipWithIndex.map {
      case (path, ix) =>
        path -> s"task-logs-$ix"
    }
  }

  private def buildTaskDefinition(family: String,
                                  executable: ECSExecutable,
                                  task: Task): ECSTaskDefinition = {
    ...
    val loggingVolumes = logPathsAndVolumes.map {
      case (path, volume) =>
        ECSVolume(volume, None)
    }
    val taskContainer = ECSContainerDefinition(
      name = "sundialTask",
      ...
      mountPoints = logPathsAndVolumes.map {
        case (path, volume) =>
          ECSMountPoint(containerPath = path, sourceVolume = volume)
      }
    )
    val companionContainer = ECSContainerDefinition(
      name = "sundialCompanion",
      ...
      essential = true,
      links = Seq.empty,
      mountPoints = hostrunMountPoint +: logPathsAndVolumes.map {
        case (path, volume) =>
  ...
  private def buildCloudWatchConfig(family: String,
                                    executable: ECSExecutable,
                                    task: Task): String = {
    val head =
      """
    ...
    val logParts = executable.logPaths.flatMap { logPath =>
      val volume =
        logPathMappings(FilenameUtils.getFullPathNoEndSeparator(logPath))
  ...
  override protected def actuallyStartExecutable(
      executable: ECSExecutable,
      task: Task)(implicit dao: SundialDao): ECSContainerState = {
    val family =
      getFamilyName(task.processDefinitionName, task.taskDefinitionName)
    ...
    val taskDefArn = taskDefArnOpt match {
      ...
      case _ =>
        ...
        val registerTaskResult =
          ecsHelper.registerTaskDefinition(desiredTaskDefinition)
    ...
    s3Client.putObject(s3Bucket,
                       s"agent-config/${task.id}",
                       new StringInputStream(cloudwatchConfig),
                       new ObjectMetadata())

    // Start the task, sending the task ID to the companion container as an environment variable
    val companionOverride = ECSContainerOverride(
      name = "sundialCompanion",
      command =
        Seq("bash",
            "-c",
            s"TASK_ID=${task.id} /opt/gilt/sundial-companion/sundial-logs.sh"))
    ...
    if (failures.nonEmpty) {
      ...
      Logger.error(
        s"Failures initializing task ${task.id} : ${reasons.mkString(",")}")
      dao.taskLogsDao.saveEvents(
        Seq(
          TaskEventLog(
            UUID.randomUUID(),
            task.id,
            new Date(),
            "executor",
            "Failed to RunTask. Got the following failures: " + reasons
              .mkString(","))))
      ECSContainerState(task.id,
                        new Date(),
                        null,
                        ExecutorStatus.Failed(Some(reasons.mkString(","))))
    } else {
  ...
  private def refreshMetadata(state: ECSContainerState)(
    ...
    try {
      val attrs = sdbClient.getAttributes(
    ...
    if (state.ecsTaskArn == null) {
      ...
      ecsTaskOpt match {
        case Some(ecsTask) =>
          ...
          state.copy(
            status = ecsStatusToSundialStatus(ecsStatus, exitCode, exitReason))
        case _ =>
          if (!state.status.isDone && !state.status
                .isInstanceOf[ExecutorStatus.Failed]) {
            state.copy(
              status = ExecutorStatus.Failed(
                Some("Couldn't find running task in ECS")))
          } else {
            state
          }
      }
  ...
  override protected def actuallyKillExecutable(
      state: ECSContainerState,
      task: Task,
      reason: String)(implicit dao: SundialDao): Unit = {
    Logger.info(
      s"Sundial requesting ECS to kill task ${task.taskDefinitionName} with Sundial ID ${task.id.toString} and ECS ID ${state.ecsTaskArn}")
    ecsHelper.stopTask(cluster, state.ecsTaskArn, reason)
  }

  private def getFamilyName(processDefinitionName: String,
                            taskDefinitionName: String): String = {
    processDefinitionName + "_" + taskDefinitionName
  }

  private def ecsStatusToSundialStatus(
      ecsStatus: String,
      exitCode: Option[Int],
    ...
    (ecsStatus, exitCode, exitReason) match {
      ...
      case ("RUNNING", _, _) =>
        ExecutorStatus.Running // The companion container may still be running
      ...
      case _ =>
        throw new RuntimeException(
          "Not sure how to parse ECS Status (" + ecsStatus + ") exitCode (" + exitCode + ")")
    }
  }

  private def getTaskExitCodeAndReason(
      ecsTask: com.amazonaws.services.ecs.model.Task)
    : (Option[Int], Option[String]) = {
    val taskContainers =
      ecsTask.getContainers.asScala.filter(_.getName == "sundialTask")
    require(taskContainers.size == 1,
            "there can only be 1 sundialTask container per task definition")
