...
case class ECSTaskDefinition(containers: Seq[ECSContainerDefinition],
                             volumes: Seq[ECSVolume],
                             family: String,
                             taskRoleArn: Option[String])
case class ECSContainerLink(sourceContainer: String, internalName: String)
case class ECSContainerDefinition(name: String,
                                  image: String,
                                  cpu: Int,
                                  memory: Int,
                                  command: Seq[String],
                                  ...
                                  environmentVariables: Map[String, String] =
                                    Map.empty,
                                  mountPoints: Seq[ECSMountPoint] = Seq.empty,
                                  links: Seq[ECSContainerLink] = Seq.empty)
...
class ECSHelper @Inject()(taskPlacementStrategy: PlacementStrategy) {
  ...
  /**
    ...
    * @return
    */
  def runTask(taskDefinition: String,
              cluster: String,
              startedBy: String,
              overrides: Seq[ECSContainerOverride] = Seq.empty)(
      implicit ecsClient: AmazonECS): RunTaskResult = {
          ...
          overrides
            .map(o =>
              new ContainerOverride()
                .withName(o.name)
                .withCommand(o.command.asJavaCollection))
            .asJavaCollection))
  ...
  def registerTaskDefinition(taskDefinition: ECSTaskDefinition)(
    ...
    val containerDefinitions = taskDefinition.containers.map { definition =>
      new ContainerDefinition()
        ...
        .withEnvironment(definition.environmentVariables
          .map {
            case (key, value) =>
        ...
        .withMountPoints(definition.mountPoints.map { mountPoint =>
          new MountPoint()
            .withContainerPath(mountPoint.containerPath)
            .withSourceVolume(mountPoint.sourceVolume)
        }.asJava)
    }
    val volumes = taskDefinition.volumes.map { volume =>
      new Volume()
        .withName(volume.name)
        .withHost(new HostVolumeProperties()
          .withSourcePath(volume.hostSourcePath.orNull))
    }
    ...
    val registerTaskDefinitionWithTaskRoleArn =
      taskDefinition.taskRoleArn.fold(registerTaskDefinitionRequest)(
        registerTaskDefinitionRequest.withTaskRoleArn(_))
  ...
  /**
    ...
    * @return
    */
  def stopTask(cluster: String, taskArn: String, reason: String)(
      implicit ecsClient: AmazonECS): StopTaskResult = {
  ...
  def listTaskDefinitionFamilies(family: String = "")(
      implicit ecsClient: AmazonECS): Set[String] = {
    val request = new ListTaskDefinitionFamiliesRequest()
  ...
  def describeTaskDefinition(family: String, revision: Option[String])(
      implicit ecsClient: AmazonECS): TaskDefinition = {
  ...
  private def asInternalModel(
      taskDefinition: TaskDefinition): ECSTaskDefinition = {
    ECSTaskDefinition(
      family = taskDefinition.getFamily,
      containers = taskDefinition.getContainerDefinitions.asScala.map {
        containerDef =>
          ECSContainerDefinition(
            ...
            environmentVariables = containerDef.getEnvironment.asScala.map {
              kvp =>
            ...
            mountPoints = containerDef.getMountPoints.asScala.map {
              mountPoint =>
      ...
      volumes = taskDefinition.getVolumes.asScala.map { volume =>
        ECSVolume(
          name = volume.getName,
          hostSourcePath =
            Option(volume.getHost).flatMap(h => Option(h.getSourcePath)))
      }.toList,
  ...
  def describeTask(cluster: String, arn: String)(
      implicit ecsClient: AmazonECS): Option[Task] = {
