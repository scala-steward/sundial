...
class PostgresDaoSpec @Inject()(dbApi: DBApi)
    extends PlaySpec
  ...
  "PostgresProcessDao" must {
    "save a process" in database.withConnection { implicit conn =>
      ...
      val process = model.Process(UUID.randomUUID(),
                                  "someProcess",
                                  new Date(),
    ...
    "load None when a process doesn't exist" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDao()
    ...
    "save process status correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDao()
        val exemplars = Seq(
          model.Process(UUID.randomUUID(),
                        "someProcess",
                        new Date(),
                        ProcessStatus.Running(),
                        Option.empty),
    ...
    "save task status correctly" in database.withConnection { implicit conn =>
      val dao = new PostgresProcessDao()
      val statuses = Seq(TaskStatus.Running(),
                         TaskStatus.Success(new Date()),
                         TaskStatus.Failure(new Date(), Some("test reason")))
      statuses.map { status =>
        val task =
          model.Task(UUID.randomUUID(),
                     UUID.randomUUID(),
    ...
    "load None when a task doesn't exist" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDao()
    ...
    "save task executables correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDao()
        val executables = Seq(
          ShellCommandExecutable("echo 'hello world'", Map.empty),
    ...
    "save a task status correctly" in database.withConnection { implicit conn =>
      val dao = new PostgresProcessDao()
      val reportedTaskStatus =
        ReportedTaskStatus(UUID.randomUUID(), TaskStatus.Success(new Date()))
      dao.saveReportedTaskStatus(reportedTaskStatus) mustBe (true)
      dao.findReportedTaskStatus(reportedTaskStatus.taskId) mustBe (Some(
        reportedTaskStatus))
      conn.commit()
    }

    "not save a task status more than once" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDao()
        val reportedTaskStatus =
          ReportedTaskStatus(UUID.randomUUID(), TaskStatus.Success(new Date()))
        ...
        dao.saveReportedTaskStatus(
          reportedTaskStatus.copy(status =
            TaskStatus.Failure(new Date(), Some("test reason")))) mustBe (false)
        dao.findReportedTaskStatus(reportedTaskStatus.taskId) mustBe (Some(
          reportedTaskStatus))
        conn.commit()
    }
  ...
  "PostgresProcessDefinitionDao" must {
    "save a process definition correctly" in database.withConnection {
      implicit conn =>
    ...
    "load None when a process definition doesn't exist" in database
    ...
    "update a process definition correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDefinitionDao()
        val definition = ProcessDefinition(
          "someDefinition",
        ...
        val updated = definition.copy(schedule = None,
                                      overlapAction =
                                        ProcessOverlapAction.Terminate,
                                      description = None,
                                      isPaused = true)
    ...
    "delete a process definition correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDefinitionDao()
        val definition = ProcessDefinition(
          "someDefinition",
    ...
    "save a task definition correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresProcessDefinitionDao()
  ...
  "PostgresTaskMetadataDao" must {
    ...
    "load an empty Seq when there's no metadata" in database.withConnection {
      implicit conn =>
        val dao = new PostgresTaskMetadataDao()
  ...
  "PostgresTriggerDao" must {
    "save and load a task trigger request correctly" in database
      .withConnection { implicit conn =>
        val dao = new PostgresTriggerDao()
        val request = TaskTriggerRequest(UUID.randomUUID(),
                                         "someProcess",
                                         "someTask",
                                         new Date(),
                                         None)
        ...
        dao.loadOpenTaskTriggerRequests(Some(request.processDefinitionName)) mustBe (Seq(
          request))
        conn.commit()
      }

    "close a task trigger request correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresTriggerDao()
        val request = TaskTriggerRequest(UUID.randomUUID(),
                                         "someProcess",
    ...
    "save and load a process trigger request correctly" in database
      .withConnection { implicit conn =>
        val dao = new PostgresTriggerDao()
        val request = ProcessTriggerRequest(UUID.randomUUID(),
                                            "someProcess",
                                            new Date(),
                                            Some(Seq("someTask")),
                                            None)
        ...
        dao.loadOpenProcessTriggerRequests(Some(request.processDefinitionName)) mustBe (Seq(
          request))
        conn.commit()
      }

    "close a task process request correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresTriggerDao()
        val request = ProcessTriggerRequest(UUID.randomUUID(),
                                            "someProcess",
    ...
    "save and load a kill request correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresTriggerDao()
        val request =
          KillProcessRequest(UUID.randomUUID(), UUID.randomUUID(), new Date())
  ...
  "PostgresShellCommandStateDao" must {
    "save and load a shell command state correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresShellCommandStateDao()
        val state = new ShellCommandState(UUID.randomUUID(),
                                          new Date(),
    ...
    "update a shell command state correctly" in database.withConnection {
      implicit conn =>
        val dao = new PostgresShellCommandStateDao()
        val state = new ShellCommandState(UUID.randomUUID(),
                                          new Date(),
